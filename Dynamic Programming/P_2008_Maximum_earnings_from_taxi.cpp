...
public:
    ...
    long long solve(int i, vector<vector<int>> &rides) {
        if (i >= rides.size())
            return 0;
        if (dp[i] != -1)
            return dp[i]; // Check memoized result
        ...
        int nextRide = lower_bound(rides.begin() + i + 1, rides.end(), rides[i][1],
                                   [](const vector<int> &ride, int target) { return ride[0] < target; }) -
                       rides.begin();
...
// Can also be done be memoising the last stop time rather then memoising the index